...
@section('title')
    @Lang('site.products') | @Lang('site.edit') {{ $product->name }}
@endsection
            ...
            <div class="row mb-2">
                ...
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        ...
                        <li class="breadcrumb-item active">@Lang('site.edit') {{ $product->name }}</li>
                ...
                <div class="col-12">
                    <div class="card card-primary">
                        <div class="card-header">
                            <h3 class="card-title">@Lang('site.edit') {{ $product->name }}</h3>
                        </div>
                                ...
                                <form action="{{ route('dashboard.products.update', $product->id) }}" method="post"
                                    ...
                                    @foreach (config('translatable.locales') as $index => $locale)
                                        <div class="form-group">
                                            ...
                                            <input type="text" name="{{ $locale }}[color]" class="color{{$index}}"
                                                value="{{ $product->translate($locale)->color }}">
                                            @error($locale . '.color')
                                                <div class="text-danger text-bold">{{ $message }}</div>
                                            @enderror
                                        </div>
                                    ...
                                    @foreach (config('translatable.locales') as $locale)
                                        <div class="form-group">
                                            ...
                                            <textarea name="{{ $locale }}[description]" rows="3"
                                                class="form-control">{{ $product->translate($locale)->description }}</textarea>
                                    ...
                                    <div id="images_upload">
                                        @foreach (json_decode($product->images, true) as $key => $image)
                                            <img src="{{ $product->image_path . '/' . $image }}" style="width: 150px"
